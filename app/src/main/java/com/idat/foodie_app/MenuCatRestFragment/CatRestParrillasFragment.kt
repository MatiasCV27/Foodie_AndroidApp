...
class CatRestParrillasFragment : Fragment() {
    ...
    private lateinit var binding: FragmentCatRestParrillasBinding

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        val view = inflater.inflate(R.layout.fragment_cat_rest_parrillas, container, false)

        binding = FragmentCatRestParrillasBinding.bind(view)
        ...
        db.collection("restaurantes").get()
            .addOnSuccessListener { documents ->
                for (document in documents) {
                    ...
                    if (wallItem.categoria == "Parrilla") {
                ...
                binding.rvRestParrillas.adapter = adapterRest
                binding.rvRestParrillas.layoutManager = LinearLayoutManager(requireContext())
            }
    }
