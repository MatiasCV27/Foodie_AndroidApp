...
import com.idat.foodie_app.databinding.FragmentCatRestTradicionalBinding

class CatRestTradicionalFragment : Fragment() {

    val db = FirebaseFirestore.getInstance()

    private lateinit var adapterRest: AdapterRestaurantes
    private lateinit var restList: ArrayList<Restaurantes>
    private lateinit var binding: FragmentCatRestTradicionalBinding

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        val view = inflater.inflate(R.layout.fragment_cat_rest_tradicional, container, false)

        binding = FragmentCatRestTradicionalBinding.bind(view)
        ...
        db.collection("restaurantes").get()
            .addOnSuccessListener { documents ->
                for (document in documents) {
                    ...
                    if (wallItem.categoria == "Tradicional") {
                ...
                binding.rvRestTradicional.adapter = adapterRest
                binding.rvRestTradicional.layoutManager = LinearLayoutManager(requireContext())
            }
    }
